...
public partial class StatisticsWindow : Window, INotifyPropertyChanged
{
    ...
    public StatisticsWindow(string databasePath)
    {
    ...
    private async void Previous_Click(object sender, RoutedEventArgs e) =>
        await LoadAsync(_month.AddMonths(-1));
    ...
    private void Apply(DesktopStatisticsSnapshot snapshot)
    {
        ...
        MonthMostPlayedValueText = month.MostPlayedGameTitle is null
            ? "—"
            : FormatDuration(month.MostPlayedGameDuration);
        MonthBusiestDayTitleText = month.BusiestDay is DateOnly busiestDay
            ? FormatDay(busiestDay)
            : "Sin actividad medida";
        MonthBusiestDayValueText = month.BusiestDay is null
            ? "—"
            : FormatDuration(month.BusiestDayDuration);

        LifetimeKnownText = FormatDuration(lifetime.KnownPlaytime);
        LifetimeBreakdownText =
            $"{FormatDuration(lifetime.MeasuredPlaytime)} medidos · {FormatDuration(lifetime.HistoricalPlaytime)} históricos";
        LifetimeGameCountText = lifetime.GameCount.ToString(CultureInfo.InvariantCulture);
        LifetimeSessionsAchievementsText =
            $"{lifetime.SessionCount.ToString(CultureInfo.InvariantCulture)} / {lifetime.UnlockedAchievementCount.ToString(CultureInfo.InvariantCulture)}";
        ...
        LifetimeMostPlayedValueText = lifetime.MostPlayedGameTitle is null
            ? "—"
            : $"{FormatDuration(lifetime.MostPlayedGameDuration)} conocidas";
        LongestSessionTitleText = lifetime.LongestSessionGameTitle ?? "Sin sesiones medidas";
        LongestSessionValueText = lifetime.LongestSessionGameTitle is null
            ? "—"
            : FormatDuration(lifetime.LongestSessionDuration);
        CurrentStreakText = lifetime.Streaks.CurrentDays == 0
            ? "Sin racha activa"
            : lifetime.Streaks.CurrentDays == 1
                ? "Racha actual · 1 día"
                : $"Racha actual · {lifetime.Streaks.CurrentDays} días";
        LongestStreakText = lifetime.Streaks.LongestDays == 0
            ? "Sin rachas registradas"
            : lifetime.Streaks.LongestDays == 1
                ? "Máxima · 1 día"
                : $"Máxima · {lifetime.Streaks.LongestDays} días";
    ...
    private static string FormatDay(DateOnly date)
    {
        var text = date.ToDateTime(TimeOnly.MinValue)
            .ToString("d 'de' MMMM", CultureInfo.CurrentCulture);
        return Capitalize(text);
    }
    ...
    private static string FormatKnownActivity(DateTimeOffset utc)
    {
        ...
        return local.ToString("d 'de' MMMM 'de' yyyy", CultureInfo.CurrentCulture);
    }
