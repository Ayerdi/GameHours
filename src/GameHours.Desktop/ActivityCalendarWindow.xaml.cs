...
public partial class ActivityCalendarWindow : Window, INotifyPropertyChanged
{
    ...
    {
        get => _monthText;
        private set => SetField(ref _monthText, value);
    }

    public string MonthSummaryText
    {
        get => _monthSummaryText;
        private set => SetField(ref _monthSummaryText, value);
    }

    public string SelectedDateText
    {
        get => _selectedDateText;
        private set => SetField(ref _selectedDateText, value);
    }

    public string SelectedDaySummaryText
    {
        get => _selectedDaySummaryText;
        private set => SetField(ref _selectedDaySummaryText, value);
    }

    public event PropertyChangedEventHandler? PropertyChanged;

    public ActivityCalendarWindow(string databasePath)
    {
    ...
    private async void Previous_Click(object sender, RoutedEventArgs e) =>
        await LoadMonthAsync(_month.AddMonths(-1));
    ...
    private async void Refresh_Click(object sender, RoutedEventArgs e) =>
        await LoadMonthAsync(_month, _selectedDate);

    private void Day_Click(object sender, RoutedEventArgs e)
    {
        if (sender is not FrameworkElement { DataContext: CalendarDayViewModel cell } ||
            cell.Day is null)
        {
            return;
        }
    ...
    private async Task LoadMonthAsync(DateOnly month, DateOnly? preferredDate = null)
    {
        ...
        try
        {
            ...
            var selected = preferredDate is DateOnly requested &&
                           requested.Year == normalized.Year &&
                           requested.Month == normalized.Month
                ? requested
