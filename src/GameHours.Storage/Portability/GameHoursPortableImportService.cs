...
public sealed class GameHoursPortableImportService
{
        ...
        CancellationToken token)
    {
        ...
        var insertSessions = new List<SessionValue>();
        ...
        var insertHistory = new List<HistoryValue>();
        ...
        foreach (var raw in achievements)
        {
            ...
            {
                conflicts.Add(new("invalid_achievement", "achievement", key, "Achievement references an unknown game."));
                continue;
            }
            if (!ValidAchievement(item, out var achievementError))
            {
                conflicts.Add(new("invalid_achievement", "achievement", key, achievementError!));
                continue;
            }
