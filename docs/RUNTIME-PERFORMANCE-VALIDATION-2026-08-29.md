...
The next distribution-level evidence gate is the signed public path. Before provisioning a signing service, confirm that the intended publisher identity is eligible for the provider's publicly trusted code-signing product. Azure Artifact Signing Public Trust is currently available to EU organizations but not to individual developers outside the United States and Canada, so the provider cannot be assumed until that publisher identity is known. Whichever provider is selected must integrate at Velopack's packaging-time signing boundary, preserve the existing Authenticode validation gate, and then be exercised through a signed `main` release plus signed install/update/recovery validation.
